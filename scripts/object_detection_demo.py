...
!pip install - q datasets transformers evaluate timm albumentations transformers[torch]
...
def find_inside_boxes(dataset_row, core_bbox):
    ...
    inside_bboxes_and_classes = [([b[0]-bbox[0], b[1] - bbox[1], (b[0]+b[2]) - bbox[0],  (b[1]+b[3]) - bbox[1]], c)
                                 ...
                                 if b[0] >= bbox[0] and b[1] >= bbox[1] and b[0]+b[2] <= bbox[2] and b[1]+b[3] <= bbox[3]]
    return [inside_bboxes for inside_bboxes, _ in inside_bboxes_and_classes], [inside_classe for _, inside_classe in inside_bboxes_and_classes], img
...
ab_ds = angebot_dataset.train_test_split(test_size=0.2)
...
checkpoint = f"/content/drive/MyDrive/Angebot/facebook-detr-resnet-50/150_epoch_run_100_images_only_products_area"
number_epochs = 150
...
training_args = TrainingArguments(
    output_dir=f"/content/drive/MyDrive/Angebot/facebook-detr-resnet-50/300_epoch_run_100_images_only_products_area",
...
latest_model_check_path = f"/content/drive/MyDrive/Angebot/facebook-detr-resnet-50/300_epoch_run_100_images_only_products_area"
...
bbx_info = [bbx for bbx in bbx_info if bbx['score'] > 0.97]
bbx_info
...
draw_bbx_on_img(img_path, bbx_info)
